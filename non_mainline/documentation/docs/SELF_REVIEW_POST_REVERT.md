...
#### ~~A1. 已知刷新内存暴涨问题未根因定位~~ → **已闭环（V3.0 / 2026-04-25）**

**初始假设修正**：用户最初判断为"刷新页面导致暴涨"，后来现场观察修正为"采集窗口期间增加快"。两个现象其实指向**同一个底层路径**——但都不是泄漏。

**验证实验**（`_capture_mem_profile.py`，5 分钟 / 5 秒粒度 / 60 个采样点）：

| 指标 | 值 | 判定 |
|------|-----|------|
| 起始 RSS | 170.5 MB | - |
| 结束 RSS | 156.2 MB | - |
| **5 分钟净变化** | **-14.3 MB** | ✅ **反向下降，绝无泄漏** |
| RSS 范围 | 147.4 ~ 198.2 MB | - |
| 摆动幅度 | 50.8 MB | 周期性尖峰 |
| GC collections | 35 → 35（不变）| 没触发 gen-2 GC |
| threads | 79 → 79 | 无线程泄漏 |
| 新增归档 | 0 | ✅ revert 干净 |

**确证结论**：
- **30 秒周期摆动**：每个 `capturing` 阶段触发瞬间 RSS 飙升 +27MB（最高 198.2），3-5 秒内回落到稳态
- **触发路径**：采集启动瞬间 OpenCV CLAHE / ECC 中间矩阵 + `device_detector` 多格栅聚合 ndarray
- **回收机制**：Python free-list（不依赖完整 GC），所以 GC counter 不变
- **历史 383MB 尖峰**（21:57，归档版本 PID 4796）的真因：归档持有 `frame_ref` ~400MB + 采集瞬时 +30MB + 当时浏览器在线 funnel_ws 推流。回退后主因消失。

**判定**：
- ❌ 不是真泄漏
- ✅ 是周期性瞬时尖峰，工业系统可接受（稳态 150-175MB，距 OOM 几个 GB 安全裕量）
- 不需要修

**可选优化**（不紧急，仅记录）：
- O1: `_finalize_window` 末尾主动 `gc.collect()` → 摆动 50MB → ~20MB
- O2: detector 内部 buffer 复用（in-place CLAHE/ECC）→ 减少瞬时 +10-15MB
- O3: `funnel_ws` 改为按需启动（无客户端时不推流）
...
| 优先级 | 项 | 工作量 | 影响 | 状态 |
|--------|-----|--------|------|------|
| ~~P0~~ | ~~A1 刷新内存暴涨~~ | - | - | ✅ 已闭环（确证为周期性尖峰，非泄漏） |
| P1 | A3 logger 级别硬编码（1 行） | 5 分钟 | 低 | 待办 |
| ~~P1~~ | ~~B3 清理临时文件~~ | - | - | ✅ G1 已完成 |
| ~~P1~~ | ~~B4 移走废弃文档~~ | - | - | ✅ G1 已完成 |
| ~~P2~~ | ~~B1 删 ImageSaver 死代码~~ | - | - | ✅ G1 已完成（commit 9724b31） |
| ~~P2~~ | ~~B2 删配置死字段~~ | - | - | ✅ G1 已完成（commit 9724b31） |
| P3 | A2 多漏斗 PLC 标签架构改造 | 1-2 周 | 高（接相机时阻塞） | 待办 |
| P3 | drivers/plc 单测补强 | 数天 | 中（覆盖盲区） | 待办 |
