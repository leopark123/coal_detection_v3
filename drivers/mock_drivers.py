...
class ImageSaver:
    ...
    def __init__(self, config):
        ...
        logger.info(f"[ImageSaver] 保存目录: {self.save_dir.absolute()}")
    
    def should_save(self, frame_id: int, is_alarm: bool) -> bool:
        """判断是否需要保存"""
        # 报警时保存
        if is_alarm and self.config.SAVE_ALARM_IMAGES:
            return True
        
        # 定时保存
        if frame_id % self.config.SAVE_INTERVAL_FRAMES == 0:
            return True
        
        return False
    
    def save(self, frame: np.ndarray, frame_id: int, is_alarm: bool = False, 
        ...
        if is_alarm:
            filename = f"ALARM_{timestamp}_frame{frame_id:06d}.jpg"
            self.alarm_count += 1
        else:
            filename = f"frame_{timestamp}_{frame_id:06d}.jpg"
        
        filepath = self.save_dir / filename
        
        # 在图像上标注检测结果
        if result:
            frame = self._annotate_result(frame.copy(), result)
        
        cv2.imwrite(str(filepath), frame)
        self.save_count += 1
        
        if is_alarm:
            logger.info(f"[ImageSaver] 🚨 保存报警图像: {filename}")
        else:
            logger.debug(f"[ImageSaver] 保存图像: {filename}")
    
    def _annotate_result(self, frame: np.ndarray, result: dict) -> np.ndarray:
        """在图像上标注检测结果"""
        # 结果文本
        texts = [
            f"Coal: {result.get('has_coal', 'N/A')}",
            f"Confidence: {result.get('confidence', 'N/A')}",
            f"Grid: {result.get('grid_visible_ratio', 0):.2%}",
            f"Coverage: {result.get('coal_coverage', 0):.2%}",
        ]
        
        # 背景
        overlay = frame.copy()
        cv2.rectangle(overlay, (5, 45), (300, 150), (0, 0, 0), -1)
        frame = cv2.addWeighted(overlay, 0.6, frame, 0.4, 0)
        
        # 文字
        for i, text in enumerate(texts):
            color = (0, 0, 255) if result.get('has_coal') else (0, 255, 0)
            cv2.putText(
                frame, text, (10, 70 + i * 25),
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "alarm_saved": self.alarm_count,
            "save_dir": str(self.save_dir.absolute())
        }
